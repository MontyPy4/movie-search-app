        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                if isinstance(result, tuple):
                    logger.info(
                        f"Query returned {len(result[0]) if result[0] else 0} results")
                return result
            except Exception as e:
...
def validate_year_range(start_year: int, end_year: int,
    ...
    if start_year > end_year:
        logger.warning(
            f"Invalid range: start_year ({start_year}) > end_year ({end_year})")
        return False

    if start_year < min_allowed or end_year > max_allowed:
        logger.warning(
            f"Year range out of bounds: {start_year}-{end_year}, allowed: {min_allowed}-{max_allowed}")
        return False
