...
def check_python_version():
    ...
    print_info(f"Текущая версия: Python {version.major}.{version.minor}.{version.micro}")
    
    if version.major >= 3 and version.minor >= 8:
        print_success("Версия Python совместима")
        return True
    else:
        print_error(f"Требуется Python 3.8 или выше (установлена {version.major}.{version.minor})")
        return False
...
def check_env_file():
    ...
    if env_file.exists():
        ...
        try:
            ...
            print_warning("Отредактируйте .env и добавьте ваши учетные данные!")
            return False
...
def run_smoke_tests():
    ...
    try:
        ...
        print_success(f"Конфигурация загружена (результатов на странице: {RESULTS_PER_PAGE})")
...
def show_next_steps():
    ...
    print(f"   - {Colors.BOLD}INSTALL.md{Colors.END} - подробная инструкция установки")
...
def main():
    ...
    if all_passed:
        ...
        if not check_database_connections():
            print_warning("Некоторые подключения не удались. Проверьте конфигурацию.")
