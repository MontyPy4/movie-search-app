...
try:
    ...
    if total_count > 0:
        ...
        keyword_docs = list(collection.find({"search_type": "keyword"}).limit(2))
        if keyword_docs:
            print(f"\n   🔍 Поиски по ключевому слову ({len(keyword_docs)} примеров):")
            for doc in keyword_docs:
                print(f"      • {doc.get('params', {}).get('keyword')} → {doc.get('results_count')} результатов")
        
        # Поиск по жанру и годам
        genre_docs = list(collection.find({"search_type": "genre__years_range"}).limit(2))
        if genre_docs:
            print(f"\n   🎬 Поиски по жанру и годам ({len(genre_docs)} примеров):")
            for doc in genre_docs:
                params = doc.get('params', {})
                genre = params.get('genre', 'N/A')
                years = params.get('years_range', 'N/A')
                print(f"      • {genre} ({years}) → {doc.get('results_count')} результатов")
